...
const uint8_t AES128Crypto::SBOX[16][16] = {
...
const uint8_t AES128Crypto::ISBOX[16][16] = {
...
const uint32_t AES128Crypto::RCON[10] = {
    0x01000000,
    0x02000000,
    0x04000000,
    0x08000000,
...
void AES128Crypto::expand_key(const ByteVector& key)
{
...
AES128Crypto::AES128Crypto(const ByteVector& key): m_w(nullptr), m_iv(16)
{
    Nk = 4, Nb = 4, Nr = 10;
    m_block_bytes = Nb * 4;
    expand_key(key.length() >= Nk * 4 ? key : key.padding(Nk * 4));
}

void AES128Crypto::encrypt_single_block(uint8_t* state) const
{
    add_round_key(state, &m_w[0]);
    for (int i = 1; i <= Nr; i++)
    {
        sub_byte(state);
        shift_row(state);
        if (i < Nr) mix_col(state);
        add_round_key(state, &m_w[i * 4]);
    }
}

AES128Crypto::~AES128Crypto()
{
    if (m_w) delete[] m_w;
}

ByteVector AES128Crypto::encrypt(const ByteVector& text) const
{
    int padding = m_block_bytes - text.length() % m_block_bytes;
    ByteVector state(text.length() + padding);
    ByteVector::copy(state, 0, text, 0, text.length());
...
ByteVector AES128Crypto::decrypt(const ByteVector& cipher) const
{
    assert(cipher.length() % m_block_bytes == 0);

    const uint8_t* last = &m_iv[0];
    ByteVector state(cipher);

    for (int i = 0; i < cipher.length(); i += m_block_bytes)
    {
        decrypt_single_block(&state[i]);
        for (int j = 0; j < m_block_bytes; j++)
            state[i + j] ^= last[j];
        last = &cipher[i];
    }

    int padding = valid_pad(state);
    if (padding < 0)
        return ByteVector::error_array();
    else
...
int AES128Crypto::valid_pad(const ByteVector& text) const
{
...
void AES128Crypto::shift_row(uint8_t* state) const
{
...
void AES128Crypto::inv_shift_row(uint8_t* state) const
{
    assert(m_block_bytes == 16);
    uint8_t tmp = state[1];
...
void AES128Crypto::decrypt_single_block(uint8_t* state) const
{
    add_round_key(state, &m_w[Nr * 4]);
    for (int i = Nr - 1; i >= 0; i--)
    {
        inv_shift_row(state);
        inv_sub_byte(state);
        add_round_key(state, &m_w[i * 4]);
        if (i > 0) inv_mix_col(state);
    }
}

void AES128Crypto::sub_byte(uint8_t* state) const
{
    for (int i = 0; i < m_block_bytes; i++)
        state[i] = Utils::sub_byte(state[i], SBOX);
}

void AES128Crypto::add_round_key(uint8_t* state, const uint32_t* roundKey) const
{
    for (int i = 0; i < Nb; i++)
    {
        for (int j = 0; j < 4; j++)
...
void AES128Crypto::inv_sub_byte(uint8_t* state) const
{
    for (int i = 0; i < m_block_bytes; i++)
...
void AES128Crypto::mix_col(uint8_t* state) const
{
    assert(Nb == 4);
...
void AES128Crypto::inv_mix_col(uint8_t* state) const
{
    assert(Nb == 4);
    for (int i = 0; i < m_block_bytes; i += 4)
    {
